...
export const analyzeSentimentAndRespond = async (message: string, history: { role: string, content: string }[]) => {
  ...
  const systemInstruction = `
    ...
    2. Provide empathetic, supportive, and motivational responses. Use a warm, friendly tone and include appropriate emojis to make the conversation feel more human and approachable. 🌟
    3. Suggest relaxation techniques (breathing, meditation, walks, etc.) when appropriate. Mention that the user can start a guided breathing or meditation session in the "Your Journey" section. 🧘‍♂️
